...
struct UserService{
    ...
    static func paginateUsers(lastCursor:String?,pageSize:UInt) async throws -> UserNode{
        ...
        let users: [UserItem] = allObjects.compactMap { usersSnapshot in
        let userDict = usersSnapshot.value as? [String:Any] ?? [:]
                return UserItem(dictionary: userDict)
            }
        if users.count == mainSnapshot.childrenCount{
            let filterdUsers = lastCursor == nil ? users : users.filter{ $0.uid != lastCursor}
